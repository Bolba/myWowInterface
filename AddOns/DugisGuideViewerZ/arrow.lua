...
function DugisArrow:Initialize()
	...
	local function PlaceWaypoint(point)
		...
		if guideIndex and DugisGuideViewer:GuideOn() and DugisGuideViewer.chardb.EssentialsMode ~= 1 then 
	...
	local function OnUpdate(self, elapsed)
		...
		if dist and dx and dy then
			...
			elseif (dist and dist <= 30 and DugisGuideViewer:ReturnTag("W")) or (dist and dist <= 5) then
				...
				if (perc > 0.96) then
					arrow:SetVertexColor(1,0.9,0)
				end
				if (perc > 0.97) then
					arrow:SetVertexColor(1,1,0.1)
				end
				if (perc > 0.98) then
					arrow:SetVertexColor(1,1,0.2)
				end
				if (perc > 0.99) then
					arrow:SetVertexColor(1,1,0.3)
				end
				if (perc > 0.995) then
					arrow:SetVertexColor(1,1,0.4)														
				end			
