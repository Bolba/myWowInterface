...
local function AddMFWaypoint(self, m, f, x, y, opts)
    ...
    if not exists then
        ...
        local zoneName = TomTomEmulation.lmd:MapLocalize(m)
        local mapId = DugisGuideViewer:GetZoneIdByName(zoneName)

        local waypoint = DGV:AddRouteWaypointWithNoTrigger(mapId, f, x, y, opts.title)
...
local function usage()
    ShowMessage("|cfff0c502Dugi|r |cff11ff11TomTom Emulation|r /way |cff11ff11Usage:|r")
    ...
    if tonumber(args[1]) ~= nil then
        ...
        if args[3] == nil then
            ...
            if x ~= nil and y ~= nil then
                local waypoint = DGV:AddRouteWaypointWithNoTrigger(map, fl, x/100, y/100, "TomTom Waypoint")
                SetCrazyArrow(waypoint)
            else
                usage()
            end
            ...
            if x ~= nil and y ~= nil then
                ...
                local waypoint = DGV:AddRouteWaypointWithNoTrigger(map, fl, x/100, y/100, description)
                SetCrazyArrow(waypoint)
            else   
                usage()
            end
                ...
                if args[1] == "list" then
                    ...
                    if zoneEnd then
                        ...
                        if mapAndDescription then
                            ...
                            if x ~= nil and y ~= nil and desc ~= nil and zoneName ~= nil then
                                if TomTomEmulation.MapExists(zoneName) then
                                    local waypoint = DGV:AddCustomRouteWaypoint(x/100, y/100, desc, zoneName, fl)
                                    SetCrazyArrow(waypoint)
                                else
                                    ShowMessage("Map "..zoneName.." doesn't exist")
                                end    
                            ...
                            if x ~= nil and y ~= nil and zoneName ~= nil then
                                if TomTomEmulation.MapExists(zoneName) then   
                                    local waypoint = DGV:AddCustomRouteWaypoint(x/100, y/100, "TomTom waypoint", zoneName, fl)
                                    SetCrazyArrow(waypoint)                                    
                                else
                                    ShowMessage("Map "..zoneName.." doesn't exist")
                                end   
...
local function HandlerWayB(msg, editbox)
    ...
    local map, fl, xCurr, yCurr = DGV:GetPlayerPosition()
    local waypoint = DGV:AddRouteWaypointWithNoTrigger(map, fl, xCurr, yCurr, "TomTom Waypoint")
    SetCrazyArrow(waypoint)
...
function UnregisterChatCommand(shortId, commandNoBackSlash)
...
function TomTomEmulation:Initialize()
    ...
    for idx, mapId in ipairs(TomTomEmulation.mapIds) do
        local mapName = TomTomEmulation.lmd:MapLocalize(mapId)
